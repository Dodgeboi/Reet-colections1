...
// Preview deployments get their own Blob prefix. This keeps branch testing
// from reading or overwriting the production catalog, orders, or site edits.
...
async function blobUrlFor(name) {
  ...
  const url = blobs.find((b) => b.pathname === pathname)?.url || null;
  ...
  if (useBlob()) {
    try {
      ...
      if (url) {
        ...
        if (r.status === 404) urlCache.delete(blobPath(name));
      }
    } catch {}
