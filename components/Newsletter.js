...
export default function Newsletter({ dark = false, compact = false }) {
  ...
  return (
    <div>
      <div className={`flex overflow-hidden rounded-full border ${dark ? "border-ivory/25 bg-white/5" : "border-onyx/15 bg-white"}`}>
        ...
        <button onClick={submit} disabled={state === "loading"} className="shrink-0 bg-gradient-to-r from-gold-light to-gold-deep px-5 font-sans text-xs font-medium uppercase tracking-wide text-onyx disabled:opacity-60">
