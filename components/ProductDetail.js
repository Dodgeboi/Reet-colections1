...
import { onSale, effectivePrice, discountPct, rating, reviewCount } from "@/lib/products";
...
export default function ProductDetail({ product, related = [] }) {
  ...
  const facts = [
    ["Fabric", product.fabric || "Pure Viscose"],
    ["Feel", "Soft & smooth"],
    ["Quality", "Lightweight & breathable"],
    ["Drape", "Flowy & elegant"],
  ];
      ...
      <div className="mx-auto max-w-6xl px-6 sm:px-8">
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <div className="lg:sticky lg:top-28 lg:self-start">
            <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-white shadow-card ring-1 ring-onyx/5">
              <Image src={product.image} alt={product.name} fill sizes="(max-width:1024px) 90vw, 45vw" className="object-cover" priority />
              {product.newIn && (
                <span className="absolute right-4 top-4 rounded-full bg-gold px-3 py-1 font-sans text-[10px] uppercase tracking-label text-onyx">New Arrival</span>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            {onSale(product) ? (
              <p className="mt-3 flex items-baseline gap-3">
                ...
                <span className="rounded-full bg-rose px-2.5 py-1 font-sans text-[11px] font-medium uppercase tracking-wide text-white">{discountPct(product)}% off</span>
              </p>
            ) : (
              <p className="mt-3 font-display text-3xl text-onyx">${product.price}</p>
            )}
            <div className="mt-3"><StarRating value={rating(product)} count={reviewCount(product)} /></div>
            ...
            {!sold && product.qty > 0 && product.qty <= 3 && (
              <p className="mt-5 inline-flex items-center gap-2 rounded-full bg-rose/10 px-3 py-1.5 font-sans text-sm font-medium text-rose-deep"><span className="text-[8px]">●</span> Only {product.qty} left — almost gone!</p>
            )}
              ...
              <div className="mt-3 flex flex-wrap gap-2.5">
                {(product.sizes || []).map((s) => (
                  <button key={s} onClick={() => { setSize(s); setErr(""); }}
                    className={`min-w-[48px] rounded-full border px-4 py-2 font-sans text-sm transition-all ${size === s ? "border-onyx bg-onyx text-ivory" : "border-onyx/20 text-onyx/70 hover:border-onyx"}`}>{s}</button>
                ))}
              </div>
            ...
            <div className="mt-7 flex items-center gap-4">
              <div className="flex items-center rounded-full border border-onyx/20">
            ...
            <button onClick={() => { if (wishlist?.toggle(product.id) === false) window.location.href = "/account"; }} className={`mt-3 flex w-full items-center justify-center gap-2 rounded-full border py-3 font-sans text-sm transition ${wished ? "border-rose bg-blush/40 text-rose-deep" : "border-onyx/20 text-onyx/70 hover:border-rose"}`}>
            ...
            {/* fabric details */}
            <div className="mt-8 rounded-2xl border border-onyx/10 bg-white p-5">
              <p className="font-sans text-[11px] uppercase tracking-label text-gold-deep">Fabric details</p>
